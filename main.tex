...
\newcommand{\dpartial}[2]{\left(\frac{\partial E_{#1}}{\partial E_{#2}}\right)}
%=================================================================
...
\emph{Rule P}: the costs per unit of exergy of the co-products are equal; “co-products” means two or more products with the same quality produced in the component. \emph{Rule F}: the costs per unit of exergy of non-exhausted and entering resources are equal.
\begin{center}
...
Third, exergy and its related cost depend on the chosen reference environment. However, the physical behavior of the plant does not depend on the analyst choice. Moreover, the right choice of the function for describing the plant is unknown: energy, exergy, (Gibbs) free energy, or any other generic function. 
...
The mathematical behavior of any component can be described through functions of the type:
\begin{equation}
    \label{eq:gcf}
    E_i=f_i(\{x\},\{E_j\}) \qquad i\in\mathcal{E}_u
\end{equation}
...
\end{equation}
which correspond to the external resources rule in the \emph{exergy cost theory}, or in other words, the cost of system's inputs are known an equal to its physical magnitude. 
...
Therefore, \cref{eq:mcost} could be expressed as follow: 
\begin{equation}
    k_{j}^{*}=\sum_{i\in\mathcal{E}_u}{k_{ij}\,k_{i}^{*}}
    \label{eq:kchar}
\end{equation}

In other words, the unit consumption establishes the cost ratio between the inputs and outputs. Moreover, the cost allocation is a basic application of the rule of the chain in the derivative calculation. Note also that \cref{eq:kchar} is independent on the characteristic equations are linear or not. The application of this equation explains the previously mentioned Rules $F$ and $P$ (as a particular case).

For example, a turbine (\cref{fig:turbine}) with the following defined efficiency is considered: 
\begin{center}
  \begin{minipage}[c]{0.50\linewidth}
   \centering
...
\begin{equation}
    ...
    \left\{\begin{array}{l}
       ...
       \dpartial{0}{5}=\dpartial{0}{3}\,\dpartial{2}{5} + \dpartial{0}{6}\,\dpartial{6}{5} \\
    ...
    \label{eq:kgenplant}
\end{equation}
\Cref{eq:egenplant} can be rewritten in the matrix form: 
\begin{equation}
    \vm{E}=\left[\vm{K}\right]\,\vm{E}+\vm{\Omega}
\end{equation}
where $\left[\vm{K}\right]$ is the matrix of unit consumption's, and $\vm{\Omega}$ the vector of system's outputs values, and \cref{eq:kgenplant} becomes:
\begin{equation}
...
Note that the matrix of the unit consumption $\left[\vm{K}\right]$ is common and transposed between \cref{eq:egenplant,eq:kgenplant}. \Cref{eq:egenplant} is the \emph{primal} of the structure and \cref{eq:kgenplant} is its dual. Every productive structure or linear input–output interpretation of a system exhibits a primal representation with a corresponding single dual and vice versa \cite{Boyd2004}.

While the arrows in the productive process point from the resources toward the products, the cost structure (or dual) point from the products toward the resources. The costs search for causes, while the products search for the objective. \cite{Valero1990d}.

\begin{figure}[ht]
    \centering
    \includegraphics{genplantr}
    \caption{Dual cost structure of generic plant}
    \label{fig:genplantr}
\end{figure}
...
E_1=k_{12}\,E_2+k_{13}\,E_3
\] 
is combined with its output equations $k_2^*=k_{12}\,k_1^*$ and $k_2^*=k_{12}\,k_1^*$,
Consequently: 
\[
...
\begin{equation}
    ...
    \label{eq:mbalcost}
\end{equation}
\Cref{eq:mbalcost} shows the \emph{cost conservation equation}. It is a mathematical consequence of the founded hypotheses.
...
\section{Results}
The presented theory mitigates several important deficiencies of the conventional exergy costing theories. First, its results do not depend on a disaggregation scheme. For example, the system in \cref{fig:genplant} is aggregated into a single component \cref{fig:genplant}.
...
The Structural theory \cite{Valero1993} has been largely ignored in the study of different alternate rules for costing allocations, such as in the allocation of the costs of wastes, by-products, or co-products. This might be due to the difficulties of determining the fundamental equations of components as in\cref{eq:gcf} or their linearized equations (e.g., \cref{eq:echar}). Rather, the theory has been used to check the logics of Rules F and P.
...
In addition, the manufacture of a continuous and broken thread costs the same exergy although the latter would not be saleable.

Exergy reduces a probably irreducible object to a single numerical value. The color, taste, form, texture, or artistic impression (i.e., properties that human beings value) have no distinguishable exergy. The exergy and exergy cost are only a measure of reality, which should not supplant other possible analyses.

Can the concept of value be assessed by thermodynamics? Does it make sense to go beyond the exergy cost when it comes to knowing the cost of objects? 

It has been claimed many times that exergy is a measure of the quality of things, which is false; it is a measure of the energy imbalance that a system has with respect to a reference environment. It is a measure of the thermodynamic quality of energy and not of objects. Moreover, the quality specification is much more complex. A dose of carbon monoxide may have the same exergy as a certain amount of food; however, everybody knows which one to ingest or burn in a boiler. The same food can present a disgusting or an extraordinary aspect and contain the same amount of exergy. Based on the many possible examples, it seems strange to confuse exergy with quality.
...
Before generalizing the problem, a simple case should be analyzed: for example, the expanding flow of a turbine produces work on the axis. In an $(h, s)$ diagram, the actual process evolves from state 1 to state 2 (\cref{fig:dt1}). 
\begin{center}
	\begin{minipage}[c]{0.48\linewidth}
		\centering
	...
	\end{minipage}
	\captionof{figure}{The degradation path of a turbine and its deterioration temperature}
	\label{fig:dt1}
\end{center}
...
If the turbine degrades, at equal to the specifications of the inflow, \#1, it is evident that an increase in the entropy generation will result in an increase in the amount of entering fuel, \#1, to keep production constant. Suppose that because of such degradation the new state of the outflow is \#2' characterized by ($h_{2'}$, $s_{2'}$). 
...
\end{align}
Subsequently, the deterioration temperature of flow \#2 is defined as a quotient:
\[
...
\begin{center}
	...
	\end{minipage}
	\captionof{figure}{Degradation path and deterioration temperature of heat source in heat exchanger}
	\label{fig:dt2}
\end{center}
In that case: 
\begin{align}
...
\]
Subsequentely \cref{eq:dt2} is also satisfied.

According to the second law, $dS_g$ is always positive; therefore, more fuel must be used to obtain the same product, and $dm > 0$. Because $T_{d,2}$ is positive, the term $(h_2-h_1 )-T_(d_2 ) (s_2-s_1 )$ must be positive; in other words, $T_{d,2} (s_1-s_2)$ is always greater than $(h_1-h_2)$.
...
\end{equation}
Combining it with \cref{eq:dt2}, results in the following equations:
\begin{equation}
\left(\frac{dm}{dS_g}\right)_r = \frac{T_0}{ex_2'-ex_2}=\frac{T_{d,2}}{\ell_1-\ell_2}
\label{eq:dmsg}
\end{equation}
Consequently, function l becomes more familiar. \Cref{eq:dmsg} presents the relationship between the increase in the entropy generation in a process in which a stream performs an activity; the amount of required additional resources depends on the following specifications: 
\begin{itemize}
	\item Constant quality of the entering resource, and
	\item Constant quality and quantity of the production,
\end{itemize}
Therefore, \cref{eq:dmsg} bacomes as follows:
\begin{equation}
...
The purpose of Thermoeconomics is to obtain a coherent and significant set of costs in a given structure. That is because its applications are directed towards the optimization and diagnosis of productive structures. Therefore, the costs of internal streams are quite important to analyze the behavior of the system. Consequently, internal costs ought to be as sensitive as possible to system degradations. A theory that would provide exact costs would be the objective of thermoeconomic diagnosis. In fact, one can obtain the costs of final products –commodities-, without performing very detailed analyses. In other words, in no way Thermoeconomics, as it is already known, is far from being finished. Moreover, the analyst needs to look for the “process of formation of cost of wastes” not mentioned here even when some ideas developed here could be used to clarify it.
...
\begin{thebibliography}{99}
	...
	\bibitem[George(2007)]{Tsatsaronis2007}
	George, T.
